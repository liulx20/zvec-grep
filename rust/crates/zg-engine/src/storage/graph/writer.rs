...
impl FileGraph {
    /// Validate against the coordinator's entity IDs before any storage mutations.
    /// The zvec file key `f{file_id}` is an implicit local endpoint.
    pub(crate) fn validate(&self, file_id: u32, local: &HashSet<&str>) -> Result<()> {
        let file_node_id = format!("f{file_id}");
        let owns = |id: &str| id == file_node_id || local.contains(id);
        for edge in &self.edges {
            nonempty(&edge.source)?;
            nonempty(&edge.target)?;
            if edge.provenance != Provenance::FileLocal
                || !owns(&edge.source)
                || !owns(&edge.target)
            {
        ...
        for reference in &self.pending_refs {
            nonempty(&reference.from_node_id)?;
...
impl SqliteGraphStorage {
    ...
    /// an empty slice for a new file. The coordinator must call `FileGraph::validate`
    /// against the replacement entities before writing. The implicit file endpoint
    /// is `f{file_id}`, matching the zvec file document key. Cross-file edges must
    /// use `apply_resolutions`.
    ...
    ///
    /// # Errors
    /// Rejects empty old IDs, read-only connections, and SQLite failures, including
    /// schema constraint violations. Failures roll back.
    pub(crate) fn write_file_graph(
        &mut self,
        file_id: u32,
        graph: &FileGraph,
        old_entity_ids: &[EntityId],
    ) -> Result<()> {
        let file_node_id = format!("f{file_id}");
        ...
        let mut targets: Vec<&str> = old_entity_ids.iter().map(EntityId::as_str).collect();
