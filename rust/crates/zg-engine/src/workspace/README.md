...
File replacement first persists `NotIndexed`, commits the SQLite graph replacement and incoming-reference invalidation, persists entities and model projections, then persists `Indexed`. Deletion first persists `Deleting`, clears graph relationships before removing entities, persists the removal, then removes and flushes the file record. `IndexStore` reads the complete old entity IDs internally before either operation. Every replacement supplies a graph snapshot; empty edge and reference collections clear the previous relationships. Graph ownership is validated directly against the canonical entities supplied in that replacement. There is no separate pending journal or automatic recovery on open. Each indexing run includes unfinished files even outside an incremental change scope; missing or excluded sources are removed, and deleting records finish deletion. Failed records retain the normal retry policy. Directories are shared immutable identities and unused directories are retained.

`graph.sqlite` lives alongside the native collections in each generation's `storage` directory and shares their handle and lock lifecycle. SQLite commits use WAL with full synchronous durability; the two backends do not share an atomic transaction. Read-only access to older indexes without a graph database leaves it absent, preserving ordinary search. Graph extraction, resolution and query integration follow separately; relationship readers and resolvers must exclude unfinished files and serialize with file updates.
