...
export type ReadHandleCacheOptions<T extends ClosableReadHandle> = {
  open: () => T | Promise<T>;
  idleTtlMs?: number;
...
/**
 * Keeps an expensive read-only handle alive between operations while making
 * invalidation wait for active readers. The cache is deliberately unaware of
 * workspace generations; callers own generation keys and replace the cache
 * when persistent state changes.
 */
export class ReadHandleCache<T extends ClosableReadHandle> {
  private handle?: T;
  ...
  constructor(private readonly options: ReadHandleCacheOptions<T>) {}

  async withRead<R>(operation: (handle: T) => Promise<R>): Promise<R> {
    if (this.closed) throw new Error("Read handle cache is closed.");
    if (this.idleTimer) {
  ...
  async close(): Promise<void> {
    if (this.closed && !this.handle && !this.openPromise) return;
    this.closed = true;
    if (this.idleTimer) {
      clearTimeout(this.idleTimer);
      this.idleTimer = undefined;
