...
export function decodeExtractBuffers(
  ...
  if (version !== KERNEL_ABI_VERSION) {
    throw new Error(
      `kernel buffer ABI ${version} != expected ${KERNEL_ABI_VERSION}`,
    );
  }
  ...
  for (let i = 0; i < nodeCount; i++) {
    const row = buffers.nodes.subarray(
      i * NODE_ROW_SIZE,
      (i + 1) * NODE_ROW_SIZE,
    );
    ...
    if (extraJson !== undefined) {
      Object.assign(node, JSON.parse(extraJson) as Partial<Node>);
    }
    nodes[i] = node;
  }
  ...
  for (let i = 0; i < edgeCount; i++) {
    const row = buffers.edges.subarray(
      i * EDGE_ROW_SIZE,
      (i + 1) * EDGE_ROW_SIZE,
    );
    ...
    const edge: Edge = {
      source:
        sourceIdx === NONE
          ? str(arena, row, EDGE.sourceIdStr)!
          : idByRow[sourceIdx]!,
      target:
        targetIdx === NONE
          ? str(arena, row, EDGE.targetIdStr)!
          : idByRow[targetIdx]!,
    ...
    if (column !== undefined) edge.col = column;
    ...
    if (metadataJson !== undefined) {
      edge.metadata = JSON.parse(metadataJson) as Record<string, unknown>;
    }
    edges[i] = edge;
  }
  ...
  for (let i = 0; i < refCount; i++) {
    const row = buffers.refs.subarray(
      i * REF_ROW_SIZE,
      (i + 1) * REF_ROW_SIZE,
    );
    ...
    const ref: UnresolvedReference = {
      fromNodeId:
        fromIdx === NONE ? str(arena, row, REF.fromIdStr)! : idByRow[fromIdx]!,
      ...
      col: row.readUInt32LE(REF.column),
    };
    if ((row.readUInt8(REF.flags) & REF_FLAG_FILE_PATH) !== 0) {
  ...
  if (errorsOff !== NONE) {
    ...
    errors = JSON.parse(
      arena.toString("utf8", errorsOff, errorsOff + errorsLen),
    ) as ExtractionError[];
  }
