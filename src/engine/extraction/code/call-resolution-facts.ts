...
export function extractCallResolutionFacts(
  ...
  const visit = (current: TSNode | null, skipSelfEntity: boolean): void => {
    ...
    const opensScope = !skipSelfEntity && opensLexicalScope(current.type, adapter.format);
...
const PARAMETER_TYPES = new Set([
  "parameter_declaration", "formal_parameter", "receiver_parameter", "parameter",
  "required_parameter", "optional_parameter", "variadic_parameter_declaration",
]);
const DECLARATION_TYPES = new Set([
  "variable_declarator", "variable_declaration", "local_variable_declaration",
  "lexical_declaration", "let_declaration", "field_declaration",
  "property_declaration", "public_field_definition", "short_var_declaration",
]);

function initialBindings(node: TSNode, adapter: LanguageAdapter): Map<string, string> {
...
function collectSignatureParameters(
  ...
  if (
    !sameSyntaxNode(current, root) && adapter.entityTypes.has(current.type) &&
    adapter.shouldIndexEntity?.(current) !== false
  ) return;
  if (PARAMETER_TYPES.has(current.type)) {
    const binding = parameterBinding(current, adapter.format);
...
function flattenScopes(scopes: readonly ReadonlyMap<string, string>[]): Map<string, string> {
  ...
  for (const scope of scopes) for (const [name, type] of scope) result.set(name, type);
  return result;
}
...
function opensLexicalScope(type: string, language: string): boolean {
  ...
  return new Set([
    "statement_block", "compound_statement", "block", "for_statement",
    "for_in_statement", "enhanced_for_statement", "while_statement", "catch_clause",
  ]).has(type);
}

function declarationBindings(node: TSNode, language: string): { name: string; type: string }[] {
  const text = node.text.trim();
  const results: { name: string; type: string }[] = [];
  const explicit = text.match(/(?:^|\b(?:const|let|var)\s+)([A-Za-z_]\w*)\s*:\s*([A-Za-z_][^\s=;,)]*)/);
  if (explicit) results.push({ name: explicit[1]!, type: normalizeType(explicit[2]!) });
  const cFamily = text.match(/^\s*([A-Za-z_][^\s=;,)]*)\s+([A-Za-z_]\w*)\s*(?:[=;,)])?/);
  if (cFamily && !/^(?:const|let|var|return|new)$/.test(cFamily[1]!))
    results.push({ name: cFamily[2]!, type: normalizeType(cFamily[1]!) });
  const constructed = text.match(/([A-Za-z_]\w*)\s*(?::=|=)\s*(?:new\s+|&)?([A-Za-z_][\w.:]*)\s*(?:\{|\(|::new)/);
  if (constructed) results.push({ name: constructed[1]!, type: normalizeType(constructed[2]!) });
  if (language === "go") {
...
function parameterBinding(node: TSNode, language: string): { name: string; type: string } | undefined {
  const name = node.childForFieldName("name")?.text ?? node.childForFieldName("pattern")?.text;
  ...
  return inferredType ? { name: match[1]!, type: normalizeType(inferredType) } : undefined;
}

function collectGenericBounds(node: TSNode, language: string): Map<string, readonly string[]> {
  const result = new Map<string, readonly string[]>();
  const genericTypes = new Set(["type_parameters", "type_parameter_list", "template_parameter_list"]);
  ...
  for (const part of inner.split(",")) {
    ...
    const constrained = language === "go"
      ? trimmed.match(/^([A-Za-z_]\w*)\s+(.+)$/)
      : language === "cpp" ? trimmed.match(/^([A-Za-z_]\w*)\s+([A-Za-z_]\w*)$/) : null;
    if (constrained && language === "go") {
      result.set(constrained[1]!, [normalizeType(constrained[2]!)]);
      continue;
    }
    if (constrained && language === "cpp" && constrained[1] !== "typename" && constrained[1] !== "class") {
...
function dispatchForLanguage(language: string): "interface" | "trait" | "virtual" {
...
function normalizeType(value: string): string {
  return value.replace(/\b(?:const|volatile|mut|typename|class)\b/g, "")
    .replace(/[&*]/g, "").replace(/<.*>|\[.*\]/g, "").trim().split(/\s+/).pop() ?? "";
}

function isCallNodeType(type: string): boolean {
  return ["call", "call_expression", "function_call_expression", "method_invocation",
    "object_creation_expression", "new_expression"].includes(type);
}
